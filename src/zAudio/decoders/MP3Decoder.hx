...
#define MAX_FREE_FORMAT_FRAME_SIZE  2304    /* more than ISO specs */
...
int8_t* DecodeMp3ToBuffer(unsigned char *buf, int music_size, uint32_t *sampleRate, uint32_t *totalSampleCount, unsigned int *channels)
{
    ...
    if (buf != NULL)
    {
        ...
        for (;;)
        {
            ...
            {
                alloc_samples *= 2;
                int8_t* tmp = (int8_t *)realloc(music_buf, alloc_samples * 2 * info.channels);
                if (tmp)
                    music_buf = tmp;
            }
            if (music_buf)
        ...
        {
            int8_t* tmp = (int8_t *)realloc(music_buf, num_samples * 2 * info.channels);
            if (tmp)
                music_buf = tmp;
        }
        ...
        if (num_samples)
            *totalSampleCount = num_samples*2;
